...
blender_args = Dict(
    :template => "$(@__DIR__)/maze_template.blend",
    :script => "$(@__DIR__)/render_maze_floor.py",
    :blender => "/spaths/bin/blender-4.2.0-linux-x64/blender"
)

function render_stims(df::DataFrame, name::String)
    out = "/spaths/datasets/$(name)/render_maze"
    isdir(out) || mkdir(out)
    scene = 1
    door = 1
    base_p = "/spaths/datasets/$(name)/scenes/$(scene)_$(door).json"
    local base_s
    open(base_p, "r") do f
        base_s = JSON.parse(f)
    end
